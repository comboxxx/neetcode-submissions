class Solution:
    def combinationSum2(self, candidates: List[int], target: int) -> List[List[int]]:
        ...
        # The Time complexity is O(n*2^n) for the worst case, the worst can happen when there are no duplicate numbers in candidates array
        # and the target value is high so we have to traverse through huge amount of callstack depth
        # The Space complexity is O(n) due to the maximum callstack depth and stack size
