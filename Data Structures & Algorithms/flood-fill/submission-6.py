class Solution:
    def floodFill(self, image: List[List[int]], sr: int, sc: int, color: int) -> List[List[int]]:
        ...
        # 4. I will declare a helper DFS function to fill in the colors recursively
        def dfs(r, c):
        ...
        # Time: O(n*m) Even though we traverse in 4 directions for each cell but we have a condition to not traverse back to the cell that we already filled the new color
        # Space: O(n*m) Even though we don't use extra auxiliary structures, space is required for the implicit recursion call stack.
