class Solution:
    def floodFill(self, image: List[List[int]], sr: int, sc: int, color: int) -> List[List[int]]:
        ...
        # Time: O(n*m) even the code logic is for traversing 4 directions for each cell but we have a condition to not traverse back to the cell that we already filled the new color
        # Space: O(n*m) even we don't have any extra auxiliary variable, but drove into recursive callstacks
