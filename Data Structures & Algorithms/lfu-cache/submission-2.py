...
class LFUCache:
    ...
    def counter(self, key):
        ...
        self.countMap[key] += 1
        # We need to remove current key from current list and add it to the higher frequency list
        self.listMap[cnt].pop(key)
        self.listMap[cnt + 1].pushRight(key)

        # If current count is equal to lfuCount and its list is empty, we need to increaes lfuCount by 1, this ensure we can evict the correct node when the memory is full
    ...
    def put(self, key: int, value: int) -> None:
        ...
        # If the key never exist in the cache and capicity is full, we need to remove the Lowest Frequent use node from the lowest count list
        if key not in self.valMap and self.cap == len(self.valMap):
            res = self.listMap[self.lfuCount].popLeft()
            self.valMap.pop(res, None)  # Remove from valMap
